...
integer :: code,nlock,i,j,k,ii,bcx,bcy,bcz,fh,ierror,jp
...
!IF (NRANK .EQ. 0) call matlab_data(uz1,uy1,uz1,fz,fy) ! write mech to be used by  matlab omar
!IF (NRANK .EQ. 0) CALL mpi_write()
!if (nrank==0 ) then 
!if (ifirst.eq.1) call save_old("time_stats.dat")

!call save_old("sauve.dat")
!call save_old("uumean.dat")
!call save_old("vvmean.dat")
!call save_old("wwmean.dat")

!call save_old("uvmean.dat")
!call save_old("uwmean.dat")
!call save_old("vwmean.dat")

!call save_old("umean.dat")
!call save_old("vmean.dat")
!call save_old("wmean.dat")

!call save_old("upert.dat")
!call save_old("vpert.dat")
!call save_old("wpert.dat")
!endif
...
!if (nrank.eq.0) then
!write(*,*) '--------------------------------------------------------'
!write(*,*) nrank, uy1(10,1,10) , size(ux1)
!write(*,*) '#########################################################'
!write(*,*) 
!endif
jp=1
...
!#################################################################
      call wallBoundary (ux1,uy1,uz1,pp3,1,1,1,1,1,1)
!#################################################################
...
!      if (nrank==1) ux1(12,64,:)=-1.
   ...
   call STATISTIC(ux1,uy1,uz1,phi1,ta1,umean,umean_temp,vmean,wmean,phimean,uumean,vvmean,wwmean,&
        uvmean,uwmean,vwmean,phiphimean,tmean)

   call vort_stats (ux1,uy1,uz1,td1,te1,tf1,tg1,th1,ti1,&
     ta2,tb2,tc2,td2,te2,tf2,di2,ta3,tb3,tc3,td3,te3,tf3,di3,vort_mean,vort_2_mean)   !!! OMAR
   call pp_stats (pp3,ta1,tb1,di1,ta2,tb2,di2,&
     ta3,di3,nxmsize,nymsize,nzmsize,phG,ph2,ph3,uvisu,p_mean,p_2_mean) 
  ...
  open (1,file='comu_stat.dat')
      write(1,*) 1./xnu,itime,ifirst,nx, ny, nz,imodulo
  close(1)
	 print*, ''

	 print*, '=========== Statistics ==================='
	call system('./stats.out')
	! CALL verif(1./xnu,itime-ifirst+1,nx, ny, nz)	
	 print*, ''
...
IF (NRANK .EQ.0) then 
  open (1,file='comu_stat.dat')
      write(1,*) 1./xnu,ilast,ifirst,nx, ny, nz,imodulo
  close(1)
	call system('./stats.out')
endif
